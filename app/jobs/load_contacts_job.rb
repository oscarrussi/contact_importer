class LoadContactsJob < ApplicationJob
  ...
  def contact_from_csv(row, user_id)
    contact = Contact.new(name: row['name'], date_of_birth: row['date_of_birth'], phone: row['phone'],
                          address: row['address'], credit_card: row['credit_card'],
                          email: row['email'], user_id: user_id)
    if contact.valid?
      contact.save
      @any_success = true
      return
    end
    failed = FailedContact.new(name: row['name'], birth: row['date_of_birth'], phone: row['phone'],
                               address: row['address'], credit_card: row['credit_card'],
                               email: row['email'], user_id: user_id, error_message: contact.errors.messages)
    failed.save
  end
end
