class Contact < ApplicationRecord
  ...
  def iso_8601_date_format
    if date_format('%Y%m%d') || date_format('%F')
      ...
      errors.add(:date_of_birth, message: 'Should use ISO 8601 (%Y%m%d or %F) format')
    end
  end
  ...
  def valid_card_number
    ...
    if brand_name.nil?
      errors.add(:credit_card, message: 'invalid credit card number')
    else
      self.franchise = brand_name
      self.card_number = JWT.encode({ credit_card_number: credit_card }, Rails.application.credentials[:secret_token],
                                    'HS256')
    end
  end
end
